...
public class PRVBookedRow implements Row {

	private final WorkListFragment mFragment;
	private final LayoutInflater mInflater;
	private final PRVForm outDTO;
	
	public PRVBookedRow(WorkListFragment fragment, PRVForm target) {
		mFragment = fragment;
	...
	public View getView(View convertView) {
        ...
        if (convertView == null) {
			vi = mInflater.inflate(R.layout.work_list_prv_tbl_booked, null);
			...
			holder.mIncompleteBtn = (Button) vi.findViewById(R.id.worklist_prv_incomplete_btn);
			
			holder.mIncompleteBtn.setOnClickListener(mFragment.INCOMPLETE_LISTENER);
			holder.mNameText.setOnClickListener(mFragment.PRV_FORM_LISTENER);
			holder.mBookText.setOnClickListener(mFragment.BOOK_LISTENER);
        ...
        SpannableString underlinedContent = new SpannableString(download.memberName.toString());
        ...
        underlinedContent = new SpannableString(download.bookedDateTime);
		underlinedContent.setSpan(new UnderlineSpan(), 0, underlinedContent.length(), 0);
        holder.mBookText.setText(underlinedContent);
		...
		holder.mIncompleteBtn.setTag(outDTO);
		holder.mBookText.setTag(outDTO);
	...
	public int getViewType() {
		return PRVWorklistAdapter.RowType.BOOKED_ROW.ordinal();
	}
